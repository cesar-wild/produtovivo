...
// ── Email: welcome message via Resend ───────────────────────────────────────
...
app.post('/webhooks/hotmart', async (req, res) => {
  ...
  if (eventType !== 'PURCHASE_APPROVED') {
    // Acknowledge non-purchase events without processing
  ...
  if (!email) {
    console.error('[hotmart] PURCHASE_APPROVED missing buyer email — skipping welcome email');
    return res.sendStatus(200);
  }
  ...
  try {
    ...
    // Don't return 500 — Hotmart will retry, causing duplicate emails
  }

  // Fire server-side Meta Purchase event for better attribution
  if (process.env.META_ACCESS_TOKEN && process.env.META_PIXEL_ID) {
    const pixelPayload = {
      data: [{
        event_name: 'Purchase',
        event_time: Math.floor(Date.now() / 1000),
        event_id: `hotmart-${purchase.transaction || Date.now()}`,
        action_source: 'website',
        user_data: { em: [email] },
        custom_data: { value: 37.00, currency: 'BRL' },
      }],
    };
    try {
...
// Manual trigger for Purchase events (e.g., from Kiwify or other platforms).
